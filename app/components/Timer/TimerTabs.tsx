...
export const TimerTabs = ({ timerMode, onModeChange }: TimerTabsProps) => {
  ...
  return (
    <div className="flex gap-1 sm:gap-1.5 p-0.5 sm:p-1 bg-white/5 rounded-xl sm:rounded-2xl mb-6 sm:mb-8 lg:mb-12 relative z-20">
      {tabs.map(({ id, label }) => {
        ...
        return (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onModeChange(id as typeof timerMode);
            }}
            className={`flex-1 px-2 sm:px-3 lg:px-6 py-2 sm:py-2.5 lg:py-3 rounded-lg sm:rounded-xl text-[10px] sm:text-xs lg:text-sm font-medium transition-all duration-300 pointer-events-auto ${
              timerMode === id
